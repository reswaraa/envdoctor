...
func TestBuiltinProbes_ContainsExpectedIDs(t *testing.T) {
	...
	if len(ps) != 4 {
		t.Fatalf("expected 4 probes; got %d", len(ps))
	}
	...
	sort.Strings(ids)
	want := []string{"docker-running", "env-required", "node-version", "port-free"}
...
func TestBuiltinProbes_NilLibAllowed(t *testing.T) {
	...
	if len(ps) != 4 {
		t.Fatalf("expected 4 probes even with nil lib; got %d", len(ps))
	}
}
